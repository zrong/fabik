...
class TestConfCommands:
    ...
    def test_conf_tpl_with_valid_files(self, temp_dir, mocker, mock_echo_functions):
        ...
        mocker.patch('fabik.cmd.GlobalState.load_conf_data', return_value=mock_fabik_config.root_data)
    ...
    def test_conf_tpl_with_nonexistent_file(self, temp_dir, mocker, mock_echo_functions):
    ...
    def test_conf_make_basic(self, temp_dir, mocker, mock_echo_functions):
        ...
        assert mock_write_config_file.call_count == 2
        mock_write_config_file.assert_has_calls([
            call("app.json", target_postfix=""),
            call("db.toml", target_postfix="")
        ])
    ...
    def test_integration_with_configreplacer(self, temp_dir, mocker):
        ...
        mock_fabik_config.getcfg.side_effect = lambda *args, **kwargs: (
            conf_data["PATH"]["tpl_dir"] if args == ("PATH", "tpl_dir") else
            conf_data["PATH"]["work_dir"] if args == ("PATH", "work_dir") else
            None
        )
...
class TestConfCommandsIntegration:
    ...
    def test_conf_tpl_integration(self, temp_dir, mocker):
        ...
        json_tpl.write_text('{"name": "{{ NAME }}", "env": "{{ ENV }}", "path": "{{ WORK_DIR }}"}')
...
''')
        ...
        mock_fabik_config = MagicMock()
        mock_fabik_config.getcfg.side_effect = lambda *args, **kwargs: (
            fabik_toml_content["PATH"]["tpl_dir"] if args == ("PATH", "tpl_dir") else
            fabik_toml_content["PATH"]["work_dir"] if args == ("PATH", "work_dir") else
            None
        )
        ...
        # 模拟 load_conf_data 方法
        mocker.patch('fabik.cmd.GlobalState.load_conf_data', return_value=fabik_toml_content)
        ...
        mock_replacer.set_writer.return_value = (temp_dir / "config.json", temp_dir / "config.json")
